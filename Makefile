.PHONY: install run rerun clean test docker-build docker-run web help
...
help:
	...
	@echo "  make web         - Start web interface (API + Frontend)"
	...
	@echo "  4. Run 'make web' to start the web interface"
...
# Start web interface (API server + Frontend)
web:
	@./start-web.sh
